...
module Jos = {
    ...
    @val @scope(("window","Jos"))
    external windowEventWithLog:(. string)=>(. string)=>(. array<string>)=>(. 'a)=>(. unit)=>unit="emitEventWithLog"
}
...
requestButton->addEventListner( "click",(event)=>{
    let success=(. _:string)=>(. result:string)=>(. )=>{
    logger(. "external")(. "Response from microapp2!")(. String(result))
}
    // let sender=Obj( ~eventFunction="onMerchantEvent",
    //     ~eventName="Process",
    //     ~payload=payload,
    //     ~receiver="microapp2")
    ...
    Jos.windowEventWithLog(. "microapp2")(. "onMerchantEvent")(. ["process"])(. success)(. )})
